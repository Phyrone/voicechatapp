...
class SnowflakeGenerator(
    ...
    private suspend fun getUniquePair(): Pair<Long, Short> {
        while (true) {
            val (time, sequence) = lastSnowflake.updateAndGet { (lastTime, lastIteration) ->
                val time = System.currentTimeMillis()
                require(lastTime <= time) { "lastTime must be before or eq time" }
                if (lastTime != time) time to 0
                else if (lastIteration >= SEQUENCE_MAX) time to SEQUENCE_MAX //lock sequence for next time
                else time to (lastIteration + 1)
            }
    ...
    companion object {
        ...
        private fun assembleSnowflake(time: Long, sequence: Short, machineID: Int): Long {
            require(sequence in 0..SEQUENCE_MAX) { "iteration must be in range 0..$SEQUENCE_MAX but is $sequence" }
            require(machineID in 0..MACHINE_ID_MAX) { "machineID must be in range 0..$MACHINE_ID_MAX but is $sequence" }
            require(time in EPOCH_START..System.currentTimeMillis()) { "time must be in range $EPOCH_START..${System.currentTimeMillis()} but is $time" }
