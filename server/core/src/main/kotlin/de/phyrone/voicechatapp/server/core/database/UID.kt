...
data class UID(
    ...
    /**
     * Encode this UID to a binary representation
     * the binary representation is build as follows:
     * 8 bytes for the [snowflake] (unsigned long)
     * followed by the [host] encoded as UTF-8
     * the byteorder is big endian
     *
     * @return binary representation of this UID
     *
     */
    fun toBinary(): ByteArray = host.toByteArray(Charsets.UTF_8).let { hostnameBytes ->
        ByteBuffer.allocate(Long.SIZE_BYTES + hostnameBytes.size).also { buffer ->
